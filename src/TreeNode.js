...
class TreeNode extends React.Component {
  ...
  _getInitialState() {
    const { verticalAnimationConfig, shouldShowAllNodes } = this.props;
  ...
  getRootNode() {
    const { depth, mapListClassName, node } = this.props;

    return (
      <ul className={mapListClassName({ depth, isOnActivePath: true, node })}>
        {this.getChildNodes(
          Object.assign({}, omit(this.props, 'node'), { nodes: node }),
        )}
      </ul>
    );
  }
  ...
  setChildrenContainerHeight(height, cb) {
    this.setState(
      {
        childrenContainerStyle: Object.assign(
          {},
          this.state.childrenContainerStyle,
          { height: `${height}` },
        ),
      },
      cb,
